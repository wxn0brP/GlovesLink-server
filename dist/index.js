...
import { clientRouter, saveSocketStatus, sseRouter, statusRouter, } from "./http.js";
    ...
    attachToHttpServer(server) {
        server.on("upgrade", async (request, socket, head) => {
            ...
            try {
                ...
                const data = url.searchParams.has("data")
                    ? JSON.parse(url.searchParams.get("data"))
                    : {};
                const authData = {
                    token,
                    data,
...
export { GLSocket, Namespace, SSESocket };
